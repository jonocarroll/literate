literate.ks.test <- function (x, y, ..., alternative = c("two.sided", "less", "greater"), 
          exact = NULL) 
{
  ...
  PVAL <- NULL
  #%{basecase}{define the base case to use}{pvalue}
  if (is.numeric(y)) {
  ...
  #%{pvalue}{calculate C_pKS2 but refer to %%basecase for reference}{returnstructure}
  ...
  #%{returnstructure}{generate the return structure}{end}
